...
fn main() -> io::Result<()> {
    ...
    // Parse the optional ending line (args[3]) to stop reading, default to starting_line + 20.
    let mut ending_line = args.get(3)
                        .and_then(|l| l.parse::<usize>().ok()) // Try to parse the string to usize, return None on error.
                        .unwrap_or(starting_line+20);                        // Use 20 if the argument is missing or parsing fails.

    if ending_line <= starting_line + 1 {
        ending_line = starting_line + 20;
    }
    ...
    println!("{} {} {} {} {}.\n\r", &args[0], &args[1], starting_line+1, ending_line, numbered);
